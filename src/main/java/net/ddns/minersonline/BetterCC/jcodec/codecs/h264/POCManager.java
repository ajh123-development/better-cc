...
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.io.model.NALUnitType;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.io.model.RefPicMarking;
...
public final class POCManager {
    ...
    public int calcPOC(final SliceHeader firstSliceHeader, final NALUnit firstNu) {
        return switch (firstSliceHeader.sps.picOrderCntType) {
            ...
            case 2 -> calcPbetter_cc(firstSliceHeader);
            default -> throw new RuntimeException("Invalid POC");
        };
    }

    private int calcPbetter_cc(final SliceHeader firstSliceHeader) {
    ...
    private boolean hasMMCO5(final SliceHeader firstSliceHeader, final NALUnit firstNu) {
        if (firstNu.type != NALUnitType.IDR_SLICE && firstSliceHeader.refPicMarkingNonIDR != null) {
            final RefPicMarking.Instruction[] instructions = firstSliceHeader.refPicMarkingNonIDR.instructions();
            for (final RefPicMarking.Instruction instruction : instructions) {
                if (instruction.type() == RefPicMarking.InstrType.CLEAR)
                    return true;
            }
        }
        return false;
