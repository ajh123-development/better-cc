...
public final class MBlockDecoderUtils {
    ...
    static void saveVect(final H264Utils.MvList mv, final int list, final int from, final int to, final int vect) {
    ...
    public static int calcMVPredictionMedian(int a, int b, int c, final int d, final boolean aAvb, boolean bAvb, boolean cAvb,
        ...
        return H264Utils.Mv.mvC(a, comp) + H264Utils.Mv.mvC(b, comp) + H264Utils.Mv.mvC(c, comp)
            - MathUtil.min3(H264Utils.Mv.mvC(a, comp), H264Utils.Mv.mvC(b, comp), H264Utils.Mv.mvC(c, comp))
            - MathUtil.max3(H264Utils.Mv.mvC(a, comp), H264Utils.Mv.mvC(b, comp), H264Utils.Mv.mvC(c, comp));
    }

    static void saveMvs(final DeblockerInput di, final H264Utils.MvList x, final int mbX, final int mbY) {
        for (int j = 0, blkOffY = mbY << 2, blkInd = 0; j < 4; j++, blkOffY++) {
            for (int i = 0, blkOffX = mbX << 2; i < 4; i++, blkOffX++, blkInd++) {
    ...
    static void savePrediction8x8(final DecoderState sharedState, final int mbX, final H264Utils.MvList x) {
