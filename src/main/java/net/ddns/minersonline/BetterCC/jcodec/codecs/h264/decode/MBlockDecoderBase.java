...
public class MBlockDecoderBase {
    ...
    public void predictChromaInter(final Frame[][] refs, final H264Utils.MvList vectors, final int x, final int y, final int comp,
								   final Picture mb, final H264Const.PartPred[] predType) {
        for (int blk8x8 = 0; blk8x8 < 4; blk8x8++) {
            for (int list = 0; list < 2; list++) {
                if (!H264Const.usesList(predType[blk8x8], list))
                    continue;
                for (int blk4x4 = 0; blk4x4 < 4; blk4x4++) {
                    final int i = H264Const.BLK_DISP_MAP[(blk8x8 << 2) + blk4x4];
                    final int mv = vectors.getMv(i, list);
                    final Picture ref = refs[list][H264Utils.Mv.mvRef(mv)];

                    final int blkPox = (i & 3) << 1;
                    final int blkPoy = (i >> 2) << 1;

                    final int xx = ((x + blkPox) << 3) + H264Utils.Mv.mvX(mv);
                    final int yy = ((y + blkPoy) << 3) + H264Utils.Mv.mvY(mv);
            ...
            final int blk4x4 = H264Const.BLK8x8_BLOCKS[blk8x8][0];
