...
import net.ddns.minersonline.BetterCC.jcodec.common.model.ColorSpace;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.io.model.*;
...
public final class SliceReader {
    ...
    final H264Const.PartPred[] predModeLeft;
    final H264Const.PartPred[] predModeTop;
    ...
    public boolean readMacroblock(final MBlock mBlock) {
        ...
        if (sh.sliceType.isInter() && !activePps.entropyCodingModeFlag) {
            ...
            if (mbSkipRun > 0) {
                ...
                predModeLeft[0] = predModeLeft[1] = predModeTop[blk8x8X] = predModeTop[blk8x8X + 1] = H264Const.PartPred.L0;
                ++mbIdx;
                return true;
            } else {
                prevMbSkipped = false;
    ...
    int readRefIdx(final boolean leftAvailable, final boolean topAvailable, final MBType leftType, final MBType topType, final H264Const.PartPred leftPred,
				   final H264Const.PartPred topPred, final H264Const.PartPred curPred, final int mbX, final int partX, final int partY, final int partW, final int partH, final int list) {
        if (!activePps.entropyCodingModeFlag)
    ...
    int readMVD(final int comp, final boolean leftAvailable, final boolean topAvailable, final MBType leftType, final MBType topType,
				final H264Const.PartPred leftPred, final H264Const.PartPred topPred, final H264Const.PartPred curPred, final int mbX, final int partX, final int partY, final int partW, final int partH,
				final int list) {
        if (!activePps.entropyCodingModeFlag)
    ...
    void read16x16DC(final boolean leftAvailable, final boolean topAvailable, final int mbX, final int[] dc) {
        if (!activePps.entropyCodingModeFlag)
            ...
            if (cabac.readCodedBlockFlagLumaDC(mDecoder, mbX, leftMBType, topMBType[mbX], leftAvailable, topAvailable,
                MBType.I_16x16) == 1)
                cabac.readCoeffs(mDecoder, CABAC.BlockType.LUMA_16_DC, dc, 0, 16, CoeffTransformer.zigzag4x4,
    ...
    int read16x16AC(final boolean leftAvailable, final boolean topAvailable, final int mbX, final int cbpLuma, final int[] ac, final int blkOffLeft,
        ...
        if (!activePps.entropyCodingModeFlag) {
            return cavlc[0].readACBlock(reader, ac, blkX, blkOffTop, blkOffLeft != 0 || leftAvailable,
                blkOffLeft == 0 ? leftMBType : MBType.I_16x16, blkOffTop != 0 || topAvailable,
            ...
            if (cabac.readCodedBlockFlagLumaAC(mDecoder, CABAC.BlockType.LUMA_15_AC, blkX, blkOffTop, 0, leftMBType,
                ...
                return cabac.readCoeffs(mDecoder, CABAC.BlockType.LUMA_15_AC, ac, 1, 15, CoeffTransformer.zigzag4x4,
                    H264Const.identityMapping16, H264Const.identityMapping16);
        }
        return 0;
    }
    ...
    int readResidualAC(final boolean leftAvailable, final boolean topAvailable, final int mbX, final MBType curMbType, final int cbpLuma,
        ...
        if (!activePps.entropyCodingModeFlag) {
            ...
            if (cabac.readCodedBlockFlagLumaAC(mDecoder, CABAC.BlockType.LUMA_16, blkX, blkOffTop, 0, leftMBType,
                ...
                return cabac.readCoeffs(mDecoder, CABAC.BlockType.LUMA_16, ac, 0, 16, CoeffTransformer.zigzag4x4,
                    H264Const.identityMapping16, H264Const.identityMapping16);
        }
        return 0;
    }
    ...
    public int readLumaAC8x8(final int blkX, final int blkY, final int[] ac) {
        final int readCoeffs = cabac.readCoeffs(mDecoder, CABAC.BlockType.LUMA_64, ac, 0, 64, CoeffTransformer.zigzag8x8,
    ...
    public void readChromaDC(final int mbX, final boolean leftAvailable, final boolean topAvailable, final int[] dc, final int comp, final MBType curMbType) {
        if (!activePps.entropyCodingModeFlag)
            ...
            if (cabac.readCodedBlockFlagChromaDC(mDecoder, mbX, comp, leftMBType, topMBType[mbX], leftAvailable,
                ...
                cabac.readCoeffs(mDecoder, CABAC.BlockType.CHROMA_DC, dc, 0, 4, H264Const.identityMapping16, H264Const.identityMapping16,
    ...
    public void readChromaAC(final boolean leftAvailable, final boolean topAvailable, final int mbX, final int comp, final MBType curMbType,
        ...
        if (!activePps.entropyCodingModeFlag) {
            ...
            if (cabac.readCodedBlockFlagChromaAC(mDecoder, blkX, blkOffTop, comp, leftMBType, topMBType[mbX],
                ...
                cabac.readCoeffs(mDecoder, CABAC.BlockType.CHROMA_AC, ac, 1, 15, CoeffTransformer.zigzag4x4,
                    H264Const.identityMapping16, H264Const.identityMapping16);
        }
    }
    ...
    public void readMBlockBDirect(final MBlock mBlock) {
        ...
        predModeTop[mbX << 1] = predModeTop[(mbX << 1) + 1] = predModeLeft[0] = predModeLeft[1] = H264Const.PartPred.Direct;
    }

    public void readInter16x16(final H264Const.PartPred p0, final MBlock mBlock) {
        final int mbX = mapper.getMbX(mBlock.mbIdx);
        final int mbY = mapper.getMbY(mBlock.mbIdx);
        final boolean leftAvailable = mapper.leftAvailable(mBlock.mbIdx);
        final boolean topAvailable = mapper.topAvailable(mBlock.mbIdx);
    ...
    private void readPredInter8x16(final MBlock mBlock, final int mbX, final boolean leftAvailable, final boolean topAvailable, final int list,
								   final H264Const.PartPred p0, final H264Const.PartPred p1) {
        final int blk8x8X = (mbX << 1);
    ...
    private void readPredictionInter16x8(final MBlock mBlock, final int mbX, final boolean leftAvailable, final boolean topAvailable,
										 final H264Const.PartPred p0, final H264Const.PartPred p1, final int list) {
        final int blk8x8X = mbX << 1;
        if (H264Const.usesList(p0, list)) {
    ...
    public void readInter16x8(final H264Const.PartPred p0, final H264Const.PartPred p1, final MBlock mBlock) {
        final int mbX = mapper.getMbX(mBlock.mbIdx);
        final int mbY = mapper.getMbY(mBlock.mbIdx);
        final boolean leftAvailable = mapper.leftAvailable(mBlock.mbIdx);
        final boolean topAvailable = mapper.topAvailable(mBlock.mbIdx);
    ...
    public void readIntra8x16(final H264Const.PartPred p0, final H264Const.PartPred p1, final MBlock mBlock) {
        final int mbX = mapper.getMbX(mBlock.mbIdx);
        final int mbY = mapper.getMbY(mBlock.mbIdx);
        final boolean leftAvailable = mapper.leftAvailable(mBlock.mbIdx);
        final boolean topAvailable = mapper.topAvailable(mBlock.mbIdx);
    ...
    private void readPredictionInter16x16(final MBlock mBlock, final int mbX, final boolean leftAvailable, final boolean topAvailable,
                                          final int list, final H264Const.PartPred curPred) {
        final int blk8x8X = (mbX << 1);
    ...
    public void readMBlock8x8(final MBlock mBlock) {
        ...
        if (mBlock.curMbType == MBType.P_8x8 || mBlock.curMbType == MBType.P_8x8ref0) {
            ...
            noSubMBLessThen8x8 = H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[0]] == 0
                && H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[1]] == 0 && H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[2]] == 0
                && H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[3]] == 0;
        }
    ...
    private void readPrediction8x8P(final MBlock mBlock, final int mbX, final boolean leftAvailable, final boolean topAvailable) {
        ...
        if (numRef[0] > 1 && mBlock.curMbType != MBType.P_8x8ref0) {
            mBlock.pb8x8.refIdx[0][0] = readRefIdx(leftAvailable, topAvailable, leftMBType, topMBType[mbX], H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0,
                mbX, 0, 0, 2, 2, 0);
            mBlock.pb8x8.refIdx[0][1] = readRefIdx(true, topAvailable, MBType.P_8x8, topMBType[mbX], H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, mbX, 2, 0, 2,
                2, 0);
            mBlock.pb8x8.refIdx[0][2] = readRefIdx(leftAvailable, true, leftMBType, MBType.P_8x8, H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, mbX, 0, 2, 2, 2,
                0);
        ...
        readSubMb8x8(mBlock, 0, mBlock.pb8x8.subMbTypes[0], topAvailable, leftAvailable, 0, 0, mbX, leftMBType,
            topMBType[mbX], MBType.P_8x8, H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, 0);
        readSubMb8x8(mBlock, 1, mBlock.pb8x8.subMbTypes[1], topAvailable, true, 2, 0, mbX, MBType.P_8x8, topMBType[mbX],
            MBType.P_8x8, H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, 0);
        readSubMb8x8(mBlock, 2, mBlock.pb8x8.subMbTypes[2], true, leftAvailable, 0, 2, mbX, leftMBType, MBType.P_8x8, MBType.P_8x8,
            H264Const.PartPred.L0, H264Const.PartPred.L0, H264Const.PartPred.L0, 0);
    ...
    private void readPrediction8x8B(final MBlock mBlock, final int mbX, final boolean leftAvailable, final boolean topAvailable) {
        final H264Const.PartPred[] p = new H264Const.PartPred[4];
        for (int i = 0; i < 4; i++) {
        ...
        for (int list = 0; list < 2; list++) {
            ...
            if (H264Const.usesList(p[1], list))
                mBlock.pb8x8.refIdx[list][1] = readRefIdx(true, topAvailable, MBType.B_8x8, topMBType[mbX], p[0],
                    predModeTop[(mbX << 1) + 1], p[1], mbX, 2, 0, 2, 2, list);
            if (H264Const.usesList(p[2], list))
                mBlock.pb8x8.refIdx[list][2] = readRefIdx(leftAvailable, true, leftMBType, MBType.B_8x8, predModeLeft[1],
                    p[0], p[2], mbX, 0, 2, 2, 2, list);
            if (H264Const.usesList(p[3], list))
                mBlock.pb8x8.refIdx[list][3] = readRefIdx(true, true, MBType.B_8x8, MBType.B_8x8, p[2], p[1], p[3], mbX, 2, 2, 2, 2,
                    list);
        }
    ...
    private void readSubMb8x8(final MBlock mBlock, final int partNo, final int subMbType, final boolean tAvb, final boolean lAvb, final int blk8x8X,
							  final int blk8x8Y, final int mbX, final MBType leftMBType, final MBType topMBType, final MBType curMBType, final H264Const.PartPred leftPred,
							  final H264Const.PartPred topPred, final H264Const.PartPred partPred, final int list) {
        switch (subMbType) {
    ...
    private void readSub8x8(final MBlock mBlock, final int partNo, final boolean tAvb, final boolean lAvb, final int blk8x8X, final int blk8x8Y, final int mbX,
							final MBType leftMBType, final MBType topMBType, final H264Const.PartPred leftPred, final H264Const.PartPred topPred, final H264Const.PartPred partPred, final int list) {
    ...
    private void readSub8x4(final MBlock mBlock, final int partNo, final boolean tAvb, final boolean lAvb, final int blk8x8X, final int blk8x8Y, final int mbX,
							final MBType leftMBType, final MBType topMBType, final MBType curMBType, final H264Const.PartPred leftPred, final H264Const.PartPred topPred,
							final H264Const.PartPred partPred, final int list) {
        mBlock.pb8x8.mvdX1[list][partNo] = readMVD(0, lAvb, tAvb, leftMBType, topMBType, leftPred, topPred, partPred,
