...
import static net.ddns.minersonline.BetterCC.jcodec.codecs.h264.decode.MBlockDecoderUtils.*;
import static net.ddns.minersonline.BetterCC.jcodec.codecs.h264.decode.PredictionMerger.mergePrediction;
...
public final class MBlockDecoderInter extends MBlockDecoderBase {
    ...
    public void decode16x16(final MBlock mBlock, final Picture mb, final Frame[][] refs, final H264Const.PartPred p0) {
        ...
        mergeResidual(mb, mBlock.ac, mBlock.transform8x8Used ? H264Const.COMP_BLOCK_8x8_LUT : H264Const.COMP_BLOCK_4x4_LUT,
    ...
    private void predictInter8x16(final MBlock mBlock, final Picture mb, final Picture[][] references, final int mbX, final int mbY,
								  final boolean leftAvailable, final boolean topAvailable, final boolean tlAvailable, final boolean trAvailable, final H264Utils.MvList x,
								  final int list, final H264Const.PartPred p0, final H264Const.PartPred p1) {
        final int xx = mbX << 2;
        ...
        final int v1 = H264Utils.Mv.packMv(mvX1, mvY1, r1);
        ...
        final int v2 = H264Utils.Mv.packMv(mvX2, mvY2, r2);
    ...
    private void predictInter16x8(final MBlock mBlock, final Picture mb, final Picture[][] references, final int mbX, final int mbY,
								  final boolean leftAvailable, final boolean topAvailable, final boolean tlAvailable, final boolean trAvailable, final int xx, final H264Utils.MvList x,
    ...
    public void decode16x8(final MBlock mBlock, final Picture mb, final Frame[][] refs, final H264Const.PartPred p0, final H264Const.PartPred p1) {
        final int mbX = mapper.getMbX(mBlock.mbIdx);
        final int mbY = mapper.getMbY(mBlock.mbIdx);
        final boolean leftAvailable = mapper.leftAvailable(mBlock.mbIdx);
        final boolean topAvailable = mapper.topAvailable(mBlock.mbIdx);
    ...
    public void decode8x16(final MBlock mBlock, final Picture mb, final Frame[][] refs, final H264Const.PartPred p0, final H264Const.PartPred p1) {
        final int mbX = mapper.getMbX(mBlock.mbIdx);
        final int mbY = mapper.getMbY(mBlock.mbIdx);
        final boolean leftAvailable = mapper.leftAvailable(mBlock.mbIdx);
        final boolean topAvailable = mapper.topAvailable(mBlock.mbIdx);
    ...
    void predictInter16x16(final MBlock mBlock, final Picture mb, final Picture[][] references, final int mbX, final int mbY,
						   final boolean leftAvailable, final boolean topAvailable, final boolean tlAvailable, final boolean trAvailable, final H264Utils.MvList x, final int xx,
        ...
        final int v = H264Utils.Mv.packMv(mvX, mvY, r);
        s.mvTopLeft.setMv(0, list, s.mvTop.getMv(xx + 3, list));
