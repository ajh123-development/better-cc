...
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.decode.ChromaPredictionBuilder;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.decode.CoeffTransformer;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.decode.Intra16x16PredictionBuilder;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.io.CAVLC;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.io.model.MBType;
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.io.write.CAVLCWriter;
import net.ddns.minersonline.BetterCC.jcodec.common.io.BitWriter;
...
import net.ddns.minersonline.BetterCC.jcodec.codecs.h264.H264Encoder;
...
public final class MBWriterI16x16 {
    public boolean encodeMacroblock(final EncodingContext ctx, final Picture pic, final int mbX, final int mbY,
									...
									final int qp, final H264Encoder.NonRdVector params) {
    ...
    private static int calcQpChroma(final int qp) {
        return H264Const.QP_SCALE_CR[MathUtil.clip(qp, 0, 51)];
    }
    ...
    private static void writeAC(final CAVLC cavlc, final BitWriter out, final int mbLeftBlk, final int mbTopBlk, final int[][] ac,
        ...
        for (int bInd = 0; bInd < ac.length; bInd++) {
            ...
            final int blkOffLeft = H264Const.MB_DISP_OFF_LEFT[bInd];
            final int blkOffTop = H264Const.MB_DISP_OFF_TOP[bInd];
    ...
    private static void writeACLum(final CAVLC cavlc, final BitWriter out, final int mbLeftBlk, final int mbTopBlk,
        ...
        if (code) {
            for (int bInd = 0; bInd < 16; bInd++) {
                final int blkOffLeft = H264Const.MB_DISP_OFF_LEFT[bInd];
                final int blkOffTop = H264Const.MB_DISP_OFF_TOP[bInd];
                nc[H264Const.BLK_DISP_MAP[bInd]] = CAVLC
    ...
    private void transform(final Picture pic, final int[][] ac, final byte[][] pred, final int x, final int y) {
        for (int i = 0; i < ac.length; i++) {
            ...
            MBEncoderHelper.takeSubtract(pic.getPlaneData(0), pic.getPlaneWidth(0), pic.getPlaneHeight(0), x + H264Const.BLK_X[i], y + H264Const.BLK_Y[i], coeff, pred[i], 4, 4);
