...
import net.ddns.minersonline.BetterCC.jcodec.common.IntArrayList;
    ...
    private static void readDecoderPicMarking(final NALUnit nalUnit, final SliceHeader sh, final BitReader _in) {
        if (nalUnit.type == NALUnitType.IDR_SLICE) {
            ...
            if (adaptiveRefPicMarkingModeFlag) {
                final ArrayList<RefPicMarking.Instruction> mmops = new ArrayList<>();
                ...
                do {
                    ...
                    final RefPicMarking.Instruction instr = switch (memoryManagementControlOperation) {
                        case 1 -> new RefPicMarking.Instruction(RefPicMarking.InstrType.REMOVE_SHORT, CAVLCReader.readUE(_in) + 1, 0); // SH: difference_of_pic_nums_minus1
                        case 2 -> new RefPicMarking.Instruction(RefPicMarking.InstrType.REMOVE_LONG, CAVLCReader.readUE(_in), 0); // SH: long_term_pic_num
                        case 3 -> new RefPicMarking.Instruction(RefPicMarking.InstrType.CONVERT_INTO_LONG,
                            CAVLCReader.readUE(_in) + 1, // SH: difference_of_pic_nums_minus1
                            CAVLCReader.readUE(_in)); // SH: long_term_frame_idx
                        case 4 -> new RefPicMarking.Instruction(RefPicMarking.InstrType.TRUNK_LONG, CAVLCReader.readUE(_in) - 1, 0); // SH: max_long_term_frame_idx_plus1
                        case 5 -> new RefPicMarking.Instruction(RefPicMarking.InstrType.CLEAR, 0, 0);
                        case 6 -> new RefPicMarking.Instruction(RefPicMarking.InstrType.MARK_LONG, CAVLCReader.readUE(_in), 0); // SH: long_term_frame_idx
                        default -> null;
                    };
                ...
                sh.refPicMarkingNonIDR = new RefPicMarking(mmops.toArray(new RefPicMarking.Instruction[]{ }));
            }
        }
    }
