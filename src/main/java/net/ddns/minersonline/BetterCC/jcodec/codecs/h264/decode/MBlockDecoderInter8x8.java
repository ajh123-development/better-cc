...
public final class MBlockDecoderInter8x8 extends MBlockDecoderBase {
    ...
    private void predict8x8P(final MBlock mBlock, final Picture[] references, final Picture mb, final int mbX, final int mbY,
                             final boolean leftAvailable, final boolean topAvailable, final boolean tlAvailable, final boolean topRightAvailable, final H264Utils.MvList x,
                             final H264Const.PartPred[] pp) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            final int blk4x4 = H264Const.BLK8x8_BLOCKS[i][0];
            PredictionMerger.weightPrediction(sh, x.mv0R(blk4x4), 0, mb.getPlaneData(0), H264Const.BLK_8x8_MB_OFF_LUMA[i], 16, 8, 8, mb.getPlaneData(0));
        }
    ...
    private void predict8x8B(final MBlock mBlock, final Frame[][] refs, final Picture mb, final int mbX, final int mbY,
        ...
        for (int list = 0; list < 2; list++) {
            ...
            if (H264Const.usesList(H264Const.bPartPredModes[mBlock.pb8x8.subMbTypes[1]], list)) {
                decodeSubMb8x8(mBlock, 1, H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[1]], refs[list], (mbX << 6) + 32,
            ...
            if (H264Const.usesList(H264Const.bPartPredModes[mBlock.pb8x8.subMbTypes[2]], list)) {
                decodeSubMb8x8(mBlock, 2, H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[2]], refs[list], mbX << 6,
            ...
            if (H264Const.usesList(H264Const.bPartPredModes[mBlock.pb8x8.subMbTypes[3]], list)) {
                decodeSubMb8x8(mBlock, 3, H264Const.bSubMbTypes[mBlock.pb8x8.subMbTypes[3]], refs[list], (mbX << 6) + 32,
    ...
    private void decodeSubMb8x8(final MBlock mBlock, final int partNo, final int subMbType, final Picture[] references, final int offX, final int offY,
								...
								final H264Utils.MvList x, final int i00, final int i01, final int i10, final int i11, final int refIdx, final Picture mb, final int off, final int list) {
        switch (subMbType) {
    ...
    private void decodeSub8x8(final MBlock mBlock, final int partNo, final Picture[] references, final int offX, final int offY, final int tl,
							  final int t0, final int tr, final int l0, final boolean tlAvb, final boolean tAvb, final boolean trAvb, final boolean lAvb, final H264Utils.MvList x, final int i00,
        ...
        final int mv = H264Utils.Mv.packMv(mBlock.pb8x8.mvdX1[list][partNo] + mvpX, mBlock.pb8x8.mvdY1[list][partNo] + mvpY, refIdx);
    ...
    private void decodeSub8x4(final MBlock mBlock, final int partNo, final Picture[] references, final int offX, final int offY, final int tl,
                              final int t0, final int tr, final int l0, final int l1, final boolean tlAvb, final boolean tAvb, final boolean trAvb, final boolean lAvb, final H264Utils.MvList x,
        ...
        final int mv1 = H264Utils.Mv.packMv(mBlock.pb8x8.mvdX1[list][partNo] + mvpX1, mBlock.pb8x8.mvdY1[list][partNo] + mvpY1, refIdx);
        ...
        final int mv2 = H264Utils.Mv.packMv(mBlock.pb8x8.mvdX2[list][partNo] + mvpX2, mBlock.pb8x8.mvdY2[list][partNo] + mvpY2, refIdx);
        ...
        interpolator.getBlockLuma(references[refIdx], mb, off + mb.getWidth() * 4, offX + H264Utils.Mv.mvX(mv2),
            offY + H264Utils.Mv.mvY(mv2) + 16, 8, 4);
    }
    ...
    private void decodeSub4x8(final MBlock mBlock, final int partNo, final Picture[] references, final int offX, final int offY, final int tl, final int t0,
							  final int t1, final int tr, final int l0, final boolean tlAvb, final boolean tAvb, final boolean trAvb, final boolean lAvb, final H264Utils.MvList x, final int i00,
							  final int i01, final int i10, final int i11, final int refIdx, final Picture mb, final int off, final int list) {
    ...
    private void decodeSub4x4(final MBlock mBlock, final int partNo, final Picture[] references, final int offX, final int offY, final int tl,
                              final int t0, final int t1, final int tr, final int l0, final int l1, final boolean tlAvb, final boolean tAvb, final boolean trAvb, final boolean lAvb, final H264Utils.MvList x,
                              final int i00, final int i01, final int i10, final int i11, final int refIdx, final Picture mb, final int off, final int list) {
        final int mvpX1 = MBlockDecoderUtils.calcMVPredictionMedian(l0, t0, t1, tl, lAvb, tAvb, tAvb, tlAvb, refIdx, 0);
        final int mvpY1 = MBlockDecoderUtils.calcMVPredictionMedian(l0, t0, t1, tl, lAvb, tAvb, tAvb, tlAvb, refIdx, 1);
        ...
        final int mv3 = H264Utils.Mv.packMv(mBlock.pb8x8.mvdX3[list][partNo] + mvpX3, mBlock.pb8x8.mvdY3[list][partNo] + mvpY3, refIdx);
        ...
        final int mv4 = H264Utils.Mv.packMv(mBlock.pb8x8.mvdX4[list][partNo] + mvpX4, mBlock.pb8x8.mvdY4[list][partNo] + mvpY4, refIdx);
        ...
        interpolator.getBlockLuma(references[refIdx], mb, off + mb.getWidth() * 4, offX + H264Utils.Mv.mvX(mv3), offY + H264Utils.Mv.mvY(mv3)
            + 16, 4, 4);
        interpolator.getBlockLuma(references[refIdx], mb, off + mb.getWidth() * 4 + 4, offX + H264Utils.Mv.mvX(mv4) + 16, offY
