...
import net.ddns.minersonline.BetterCC.jcodec.common.ArrayUtil;
...
public final class Intra16x16PredictionBuilder {
    ...
    public static void predictVertical(final int[][] residual, final byte[] topLine, final int x, final byte[] pixOut) {
        ...
        for (int j = 0; j < 16; j++) {
            for (int i = 0; i < 16; i++, off++)
                pixOut[off] = (byte) MathUtil.clip(residual[H264Const.LUMA_4x4_BLOCK_LUT[off]][H264Const.LUMA_4x4_POS_LUT[off]] + topLine[x + i],
                    -128, 127);
        }
    }
    ...
    public static void predictHorizontal(final int[][] residual, final byte[] leftRow, final byte[] pixOut) {
        ...
        for (int j = 0; j < 16; j++) {
            for (int i = 0; i < 16; i++, off++)
                pixOut[off] = (byte) MathUtil.clip(residual[H264Const.LUMA_4x4_BLOCK_LUT[off]][H264Const.LUMA_4x4_POS_LUT[off]] + leftRow[j], -128,
                    127);
        }
    }
